...
def CLOUDY_RUN(Lumin,Column_density_order,multi_factor):
    ...
    def CLOUDY_QSO(Lumin,Column_density_order,multi_factor):

        folder_path = os.path.join(path, f"Lum_{Lumin}_1/N_H_{multi_factor}_{Column_density_order}", "w_LT/QSO")  # ← '/' 제거
        ...
        folder_path = os.path.join(path, f"Lum_{Lumin}_2/N_H_{multi_factor}_{Column_density_order}", "w_LT/QSO")  # ← '/' 제거
        os.makedirs(folder_path, exist_ok=True)
    ...
    def CLOUDY_QSO_Masking(Lumin,Column_density_order,multi_factor):
    ...
    #--------------------------------------------------------------------------------------------------------
 # ← '/' 제거
        folder_path = os.path.join(path, f"Lum_{Lumin}_3/N_H_{multi_factor}_{Column_density_order}", "w_LT/QSO_M")
        os.makedirs(folder_path, exist_ok=True)
    ...
    #--------------------------------------------------------------------------------------------------------
        folder_path = os.path.join(path, f"Lum_{Lumin}_4/N_H_{multi_factor}_{Column_density_order}", "w_LT/QSO_M")
        os.makedirs(folder_path, exist_ok=True)
    ...
    CLOUDY_QSO(Lumin,Column_density_order,multi_factor)
    CLOUDY_QSO_Masking(Lumin,Column_density_order,multi_factor)
...
for jj, Column_density_order in enumerate(N_H_odrer):
    for kk , mul in enumerate(multi_factor):
        for ii, Lumin in enumerate(Lum):
            if Lumin == 45.5 or Lumin == 43.5:
                Lumin = Lumin
                CLOUDY_RUN(Lumin,Column_density_order,mul)
            else: 
                Lumin = int(Lumin)
                CLOUDY_RUN(Lumin,Column_density_order,mul)